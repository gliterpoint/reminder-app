var calendar = (function () {
  ...
  var confingMap = {
    inputSection:
      ...
      "<div id = 'displayMonth' class = 'displayMonth'>" +
      dateLib.months()[dateLib.today.getMonth() + 1].name +
      "<br>" +
      dateLib.today.getFullYear() +
      "</div>" +
      "</header>" +
      "<main>" +
      "<div class = 'week'>" +
      ...
      "</main>" +
      `<h4>
      <p id="credit">© all rights reserved to <a href="https://github.com/DvirYotvat" target="_blank">Dvir Yotvat</a>.
      </p></h4>`,
  };
  ...
  var fillMonth = function (date, events = undefined) {
    ...
    var startY =
      table[0].type == "PMday"
        ? displayDate.month - 1 == 0
          ? displayDate.year - 1
          : displayDate.year
        : displayDate.year;
    var endY =
      table[table.length - 1].type == "NMday"
        ? displayDate.month + 1 == 13
          ? displayDate.year + 1
          : displayDate.year
        : displayDate.year;
      ...
      $(".CMday .dNum")
        .filter(function () {
          return (
            $(this)
              .text()
              .replace(/[^\d.]/g, "") == date.day
          );
        })
        .attr("id", "current_Day_Num");
      $("#current_Day_Num").parent().attr("id", "current_Day");
      $("#current_Day_Num").css({ "background-color": "#1a60bc" });
    }
    $(".CMday,.PMday,.NMday,.current_Day").css({
      width: 100 / 7 + "%",
      height: "100%",
    });
    ...
    var allEvent = {
      daily: localMemoryAPI.getAll("daily"),
      weekly: localMemoryAPI.getAll("weekly"),
      monthly: localMemoryAPI.getAll("monthly"),
    };

    //list of events that need to be display
    eventList = eventInDisplayMonth(
      allEvent["daily"],
      allEvent["weekly"],
      allEvent["monthly"],
      mStartDate,
    ...
    $(".PMday,.CMday,.NMday").click(function (e) {
      ...
      if (
        $(e.target).attr("class") !== "CMday" &&
        $(e.target).attr("class") !== "PMday" &&
        $(e.target).attr("class") !== "NMday"
      ) {
        if ($(e.target).attr("class") == "dNum") {
          if (
            $(e.target).parents(".PMday,.CMday,.NMday").attr("class") == "PMday"
          ) {
            clickedYear = clickedMonth == 1 ? clickedYear - 1 : clickedYear;
            clickedMonth = clickedMonth == 1 ? 12 : clickedMonth - 1;
          } else if (
            $(e.target).parents(".PMday,.CMday,.NMday").attr("class") == "NMday"
          ) {
        ...
        clickedDay = $(e.target.getElementsByClassName("dNum"))
          .text()
          .replace(/[^\d.]/g, "");
      }
      ...
      $("#eventInTD table").empty();
      $("#dayHeader").append(
        clickedDay + "/" + clickedMonth + "/" + clickedYear
      );
      var temp;
      for (var i = 0; i < pressed.length; i++) {
        ...
        temp.click(function (e) {
          ...
          target.find("#edit").click(function () {
            ...
            $("#switch_weekly,#switch_monthly,#switch_daily").attr(
              "disabled",
              true
            );
            ...
            $("#ok").click(function () {
              ...
              switch (hitEvent.type) {
                ...
                case "weekly":
                  ...
                  hitEvent.endDate = dateLib.endDate(
                    getDate($("#beginD").val()),
                    { type: "weeks", amount: $("#numrepeat").val() }
                  );
                  hitEvent.startTime = getTime($("#beginD").val());
                ...
                case "monthly":
                  ...
                  hitEvent.endDate = dateLib.endDate(
                    getDate($("#beginD").val()),
                    { type: "months", amount: $("#numrepeat").val() }
                  );
                  hitEvent.startTime = getTime($("#beginD").val());
                  hitEvent.endTime = getTime($("#beginD").val());
                  hitEvent.eventColor = $("#colorful").val();
              ...
              $("#switch_weekly,#switch_monthly,#switch_daily").prop(
                "disabled",
                false
              );
              $("#switch_weekly,#switch_monthly,#switch_daily").prop(
  ...
  var monthView = function (date) {
    ...
    var day =
      date.month - 1 == 0
        ? months[12].days - (dayInWeek - 2)
        : months[date.month - 1].days - (dayInWeek - 2);

    //fill prev month days in the array
    for (var i = 0; i < dayInWeek - 1; i++) {
      table[i] = { day: day, type: "PMday", eventToday: [] };
      day++;
    }
    ...
    for (; day <= months[date.month].days; i++, day++) {
      table[i] = { day: day, type: "CMday", eventToday: [] };
      if (day == 1)
    ...
    day = 1;
    for (; i < weeks * 7; i++, day++) {
      table[i] = { day: day, type: "NMday", eventToday: [] };
      if (day == 1)
        //first day of the month add name attribute
        table[i]["month"] =
          date.month == 12 ? months[1].name : months[date.month + 1].name;
    }
    return table;
  };
...
function eventInDisplayMonth(
  ...
  list.sort(function (a, b) {
    var c1 =
      a.startDate.year +
      (a.startDate.month < 10 ? "0" + a.startDate.month : a.startDate.month) +
      (a.startDate.day < 10 ? "0" + a.startDate.day : a.startDate.day) +
      a.startTime;
    var c2 =
      b.startDate.year +
      (b.startDate.month < 10 ? "0" + b.startDate.month : b.startDate.month) +
      (b.startDate.day < 10 ? "0" + b.startDate.day : b.startDate.day) +
      b.startTime;
  ...
  list.sort(function (a, b) {
    // check wich task be first if the tow tasks will start in the same time
    if (a.startTime === b.startTime) {
      // add hours of the task to the end time
      let tempA_endTime = parseInt(a.startTime) + parseInt(a.taskHoures);
      // add hours of the task to the end time
      let tempB_endTime = parseInt(b.startTime) + parseInt(b.taskHoures);
      if (tempA_endTime < tempB_endTime) {
        // check how many digits the number has and update the end time of the task
        if (String(Math.abs(tempA_endTime)).charAt(0) == tempA_endTime) {
          a.endTime = `0${tempA_endTime}:${a.endTime.substring(
            3,
            a.endTime.length
          )}`;
        } else {
          a.endTime = `${tempA_endTime}:${a.endTime.substring(
            3,
            a.endTime.length
          )}`;
        }
        ...
        tempB_endTime = parseInt(b.startTime) + parseInt(b.taskHoures);
        if (String(Math.abs(tempB_endTime)).charAt(0) == tempB_endTime) {
          b.endTime = `0${tempB_endTime}:${b.endTime.substring(
            3,
            b.endTime.length
          )}`;
        } else {
          b.endTime = `${tempB_endTime}:${b.endTime.substring(
            3,
            b.endTime.length
          )}`;
        }
      } else if (tempB_endTime < tempA_endTime) {
        // check how many digits the number has and update the end time of the task
        if (String(Math.abs(tempB_endTime)).charAt(0) == tempB_endTime) {
          b.endTime = `0${tempB_endTime}:${b.endTime.substring(
            3,
        ...
        tempA_endTime = parseInt(a.startTime) + parseInt(a.taskHoures);
        if (String(Math.abs(tempA_endTime)).charAt(0) == tempA_endTime) {
          a.endTime = `0${tempA_endTime}:${a.endTime.substring(
            3,
            a.endTime.length
  ...
  // sort by split
  list.sort(function (a, b) {
    // case task a and task b are cant be split
    if (a.startTime === b.startTime) {
      // add hours of the task to the end time
      let tempA_endTime = parseInt(a.startTime) + parseInt(a.taskHoures);
      // add hours of the task to the end time
...
function reSize() {
  ...
  var bodyScale = 0.900497833935018;
  ...
  $("#nextMonth").css({
    "font-size": $("header").height() * npMFontScale + "px",
  });
  $("#prevMonth").css({
    "font-size": $("header").height() * npMFontScale + "px",
  });
  $("#displayMonth").css({
    "font-size": $("header").height() * dmFontScale + "px",
  });
  $(".weekDay").css({ "font-size": $(".week").height() * weekDScale + "px" });

  monthTdResize();

  if ($("#Event").is(":visible")) sideBarResize();
}
...
function sideBarResize() {
  ...
  $("#from").each(function () {
    $(this).css({
      "font-size": $("#eventInTD table").width() * dFontScale + "px",
    });
  });
  $("#to").each(function () {
    $(this).css({
  ...
  $(".tdSizeEvent").css({
    width: $("#eventInTD").width() * tdEventScale + "px",
  });
}
